...
No lint, format, test, or typecheck scripts are configured.
...
- **Routes** (`src/routes/`): Auto-loaded. Subdirectories with `index.ts` become route modules. Files export a Fastify plugin as default.
...
- TypeScript config: `strict: true`, `target: ES2023`, `module: ESNext`, `moduleResolution: bundler`.
